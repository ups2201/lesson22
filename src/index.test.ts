...
import {
    ...
    getOutcomeCashCategoriesCodeFromPeriod, getSumOperations, sortCategoryForPeriod, getConsumptionByDatesFromRange
} from "./index.ts";
...
describe('Функции', () => {
    ...
    test('Проверка получения операций за указанный период', () => {
        ...
        expect(getOperationFromPeriod(operations, startDate, endDate)).toEqual(expectedResult);
    });
    ...
    test('Проверка расход по категориям из диапозона дат', () => {
        ...
        const map:Map<Category, Array<Operation>> = getFullMapCategoryForPeriod(operations, categories, startDate, endDate);
        const mapResult: Map<string, Number> = getSumMapFromCategory(map, startDate, endDate);
    ...
    test('Сортировка категорий по сумме за указанный диапозон дат', () => {
        ...
        expect(sortCategoryForPeriod(map, startDate, endDate)).toStrictEqual(expectedResult);
    });
    ...
    test('Расход по датам из диапазона', () => {
        ...
        expect(getConsumptionByDatesFromRange(operations, startDate, endDate)).toEqual(expectedResult);
    });
    ...
    test('Получаем список кодов расходных операций за определённый период', () => {
        ...
        expect(getOutcomeCashCategoriesCodeFromPeriod(operations, "ЖКХ", startDate, endDate)).toEqual(new Set([333,444]));
    });
